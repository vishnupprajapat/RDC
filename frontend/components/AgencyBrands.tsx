...
export const AgencyBrands: React.FC<FocusedAgenciesProps> = ({ focusedAgencies }) => {
    ...
    return (
        <section id="agencies" className="relative isolate overflow-hidden min-h-screen w-full flex items-center justify-center py-20 md:py-28">
            {/* 🔹 Background Video */}
            <video
            ...
            <div className="relative z-20 mx-auto sm:pb-10 container  text-center ">
                ...
                {focusedAgencies.agencies &&
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-20 xl:gap-40">
                        {/* CMS MINDS */}
                        {focusedAgencies.agencies.map((agency: any) => {
                            ...
                            return (
                                ...
                                >
                                    <div className="relative w-full xl:w-[575px] h-[250px] bg-white/50 border border-[#000F1A33] rounded-[20px] backdrop-blur-[30px] flex items-center justify-center transition-all duration-200 ease-in-out">
                                        {image && (
                                        <Image
                                        ...
                                        <span className={`text-black absolute ${agency.name === "Agency Minds"?"right-[30px]":"right-[50px]"}  hidden lg:block text-[50px] sm:text-3xl opacity-0 group-hover:opacity-100  transition-all duration-300 ease-in-out`}>
                                            <HiArrowUpRight className="w-11 h-11 text-black" />
                                        </span>
                                    </div>

                                    <h3 className="text-[24px] md:text-[28px] lg:text-[34px]  font-work-sans font-semibold mt-6 text-center"
                                      dangerouslySetInnerHTML={{ __html: agency.tagline }}
                                    />
                                     
                                    <p className="mt-4 sm:text-lg md:text-xl text-[20px] font-work-sans font-normal  text-[#000F1A] text-center">
                                        {agency.description}
                                    </p>
                                </Link>
                            );
